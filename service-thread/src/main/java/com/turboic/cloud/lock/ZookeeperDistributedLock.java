package com.turboic.cloud.lock;
...
public class ZookeeperDistributedLock extends AbstractDistributedLock {
     ...
     * @return 锁对象
     */
    public static ZookeeperDistributedLock create(String lockName, String zkUrl) {
        ZookeeperDistributedLock ZookeeperDistributedLock = new ZookeeperDistributedLock();
        if (lockName != null && lockName.length() > 0) {
            ZookeeperDistributedLock.lockName = lockName;
        }
        ZookeeperDistributedLock.init(zkUrl);
    ...
    public static ZookeeperDistributedLock create(String zkUrl) {
